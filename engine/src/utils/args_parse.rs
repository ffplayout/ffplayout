...
pub async fn run_args(pool: &Pool<Sqlite>) -> Result<(), ProcessError> {
    ...
    if args.init {
        ...
        channel.storage.original_path = global.storage;
        let mut storage_path = PathBuf::from(channel.storage.original_path.clone());
        ...
        if global.shared {
            ...
            channel.playlists = Path::new(&channel.playlists)
                ...
                .to_string_lossy()
                .to_string();
            channel.storage.original_path = storage_path.to_string_lossy().to_string();
        };
