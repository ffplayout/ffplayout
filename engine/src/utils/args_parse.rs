...
pub async fn run_args(pool: &Pool<Sqlite>) -> Result<(), i32> {
    ...
    if args.init {
        ...
        channel.storage.raw_path = global.storage;
        let mut storage_path = PathBuf::from(channel.storage.raw_path.clone());
        ...
        if global.shared {
            ...
            channel.playlists = Path::new(&channel.playlists)
                ...
                .to_string_lossy()
                .to_string();
            channel.storage.raw_path = storage_path.to_string_lossy().to_string();
        };
