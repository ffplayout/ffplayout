...
SET output_mode_old = output_mode;

ALTER TABLE configurations DROP COLUMN output_mode;

ALTER TABLE configurations
ADD COLUMN output_mode INTEGER NOT NULL DEFAULT 0;

INSERT INTO outputs (name, parameters, channel_id)
SELECT 'hls',
...
SET output_mode = (
    SELECT outputs.id
    FROM outputs
    WHERE outputs.name = configurations.output_mode_old
      AND outputs.channel_id = configurations.channel_id
)
WHERE output_mode_old IS NOT NULL;

UPDATE outputs
SET parameters = (
    SELECT configurations.output_param
    FROM configurations
    WHERE configurations.output_mode = outputs.id
...
ALTER TABLE configurations DROP COLUMN output_mode_old;
ALTER TABLE configurations DROP COLUMN output_param;
